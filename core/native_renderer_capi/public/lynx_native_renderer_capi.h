...
// One list item's action entry for `lynx_element_update_list_actions`:
...
// Items mentioned in NO action keep their identity, matching how
// ReactLynx drives the same adapter.
//
// Index contract (what `AdapterHelper` expects; it applies
...
//   - `inserts`: ascending splice points into the POST-removal list,
//     applied in array order. Entries carry the per-item layout
//     metadata — the ONLY channel the adapter ingests it from
//     (list-item element attributes are NOT read by the decoupled
//     list).
